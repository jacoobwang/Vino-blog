 ...
 * Time: 14:49
 */
class Db extends ConsoleHelper {
    ...
    {
        try {
            $conn = new \PDO($this->_config["dsn"], $this->_config["username"], $this->_config["password"]);
            if ($conn) {
                $this->stdout("   DB is ok \n", Console::FG_RED);
            }
        } catch (\PDOException $e) {
            $this->stdout("   DB error: " . $e->getMessage(). "\n", Console::FG_RED);
            if (strpos($e->getMessage(), "vino_blog")) {
                $this->stdout("   Run php vino-cli db init to install database", Console::FG_RED);
            } else {
                $this->stdout("   Modify your db config with file:". SITE_ROOT."/configs/db.php \n");
            }
        }
    }
    ...
    /**
     * loading config
     * @throws \Vino\Exception
     */
    private function loadConfig()
    {
        $config = new \Vino\Config(SITE_ROOT."/configs");
        $this->_config = $config->get("db/m");
    }

    /**
     * default function indx
     */
    public function actionIndex()
    {
        $this->loadConfig();
        $this->stdout("Your db config is readding :\n");
        foreach($this->_config as $value) {
            $this->stdout("   {$value}\n", Console::FG_YELLOW);
        }
        $this->stdout("Testing you db config :\n");
        $this->getConnection();
    }
    ...
    {
        $this->loadConfig();
        $con = mysqli_connect("localhost",$this->_config["username"], $this->_config["password"]);

        if (!$con)
        {
            $this->stdout("连接错误: " . mysqli_connect_error() ."\n");
            exit;
        }
     ...
     * @param $link
     */
    private function createDatabase($link)
    {
        mysqli_query($link, "CREATE DATABASE IF NOT EXISTS `vino_blog`  DEFAULT CHARACTER SET utf8 COLLATE utf8_unicode_ci");
        $this->stdout("Create Database vino_blog \n");
    }
    ...
    private function createTables()
    {
        ...
        $sql = [
            "CREATE TABLE `vino_blog_cate` (`term_id` bigint(20) unsigned NOT NULL AUTO_INCREMENT,`name` varchar(200) NOT NULL DEFAULT '',`slug` varchar(200) NOT NULL DEFAULT '',`term_group` bigint(10) NOT NULL DEFAULT '0',PRIMARY KEY (`term_id`),KEY `name` (`name`),KEY `slug` (`slug`)) ENGINE=InnoDB AUTO_INCREMENT=10 DEFAULT CHARSET=utf8;",
            "CREATE TABLE `vino_blog_label` (`id` bigint(20) unsigned NOT NULL AUTO_INCREMENT,`post_id` bigint(20) unsigned NOT NULL DEFAULT '0',`label` varchar(255) NOT NULL DEFAULT '',`type` varchar(20) NOT NULL DEFAULT '',PRIMARY KEY (`id`),KEY `post_id` (`post_id`)) ENGINE=InnoDB AUTO_INCREMENT=85 DEFAULT CHARSET=utf8;",
            "CREATE TABLE `vino_blog_posts` (`id` bigint(20) unsigned NOT NULL AUTO_INCREMENT,`post_author` bigint(20) unsigned NOT NULL DEFAULT '0',`post_date` datetime NOT NULL DEFAULT '0000-00-00 00:00:00',`post_content` longtext NOT NULL,`post_title` varchar(1000) NOT NULL DEFAULT '',`post_status` varchar(20) NOT NULL DEFAULT 'publish',`post_modified` datetime NOT NULL DEFAULT '0000-00-00 00:00:00',`post_type` varchar(20) NOT NULL DEFAULT 'post',`post_md` longtext,`post_thumb` varchar(2000) NOT NULL DEFAULT '',`post_desc` varchar(2000) NOT NULL DEFAULT '',`post_origin` varchar(2000) NOT NULL DEFAULT '',PRIMARY KEY (`id`),KEY `post_author` (`post_author`),KEY `type_status_date` (`post_type`,`post_status`,`post_date`)) ENGINE=InnoDB AUTO_INCREMENT=14 DEFAULT CHARSET=utf8;",
            "CREATE TABLE `vino_blog_setting` (`id` smallint(2) unsigned NOT NULL AUTO_INCREMENT,`site_title` varchar(255) NOT NULL DEFAULT '',`site_desc` varchar(255) NOT NULL DEFAULT '',`site_email` varchar(255) NOT NULL DEFAULT '',`site_logo` varchar(255) NOT NULL DEFAULT '',`site_thumbnail` varchar(255) NOT NULL DEFAULT '',`banner1` varchar(255) NOT NULL DEFAULT '',`banner2` varchar(255) NOT NULL DEFAULT '',`banner3` varchar(255) NOT NULL DEFAULT '',`banner4` varchar(255) NOT NULL DEFAULT '',`register_open` smallint(2) NOT NULL DEFAULT '1',PRIMARY KEY (`id`)) ENGINE=InnoDB AUTO_INCREMENT=2 DEFAULT CHARSET=utf8;",
            "CREATE TABLE `vino_blog_users` (`id` int(10) unsigned NOT NULL AUTO_INCREMENT,`user_login` varchar(255) NOT NULL DEFAULT '',`user_pass` varchar(255) NOT NULL DEFAULT '',`user_nickname` varchar(255) NOT NULL DEFAULT '',`user_email` varchar(255) NOT NULL DEFAULT '',`user_registered` datetime NOT NULL DEFAULT '0000-00-00 00:00:00',`user_avatar` varchar(255) DEFAULT '',`user_profile` varchar(2000) NOT NULL DEFAULT '',`user_power` varchar(20) NOT NULL DEFAULT 'guest',`user_github` varchar(255) NOT NULL DEFAULT '',`user_weibo` varchar(255) NOT NULL DEFAULT '',PRIMARY KEY (`id`),KEY `user_login` (`user_login`)) ENGINE=InnoDB AUTO_INCREMENT=2 DEFAULT CHARSET=utf8;",
        ...
        $this->stdout("Database is prepareing ok,Run php vino-cli server to see your blog \n");
    }
}
